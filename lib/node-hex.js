...
class Nodehex {
    ...
    async table(type) {
        return await table.get(type)
    }

    async hexName(hexCode) {

        let dataTable = await table.getAll();

        for await (let i of dataTable) {
            for await (let ii of i) {

                if (hexCode.toString().toUpperCase()===ii.hex.toString().toUpperCase()) return ii.name
